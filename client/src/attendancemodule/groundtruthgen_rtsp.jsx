...
const CAMERAS = [
  {
    id: 'cam_main',
    label: 'Main Hall — Front',
    url: 'rtsp://127.0.0.1:8554/live',
  },
  {
    id: 'cam_side',
    label: 'LT103L',
    url: 'rtsp://admin:Admin%401234%23@10.10.177.249:554/video/live?channel=1&subtype=0&rtsp_transport=tcp',
  },
  {
    id: 'cam_lab1',
    label: 'LT103R',
    url: 'rtsp://admin:Admin%401234%23@10.10.177.250:554/video/live?channel=1&subtype=0&rtsp_transport=tcp',
  },
...
const DET_SIZE_OPTIONS = [
  {
    value: 320,
    label: 'Fast (320)',
    hint: '~4× faster, good for clear footage',
  },
  {
    value: 640,
    label: 'Accurate (640)',
    hint: 'Better for small/distant faces',
  },
];
...
const PersonCard = ({ id, count, target }) => {
  ...
  return (
    ...
    >
      <div
        style={{ fontSize: '11px', color: theme.textMuted, marginBottom: 4 }}
      >
        {id}
      </div>
      <div
      ...
      >
        {count}
        <span
          style={{ fontSize: '12px', fontWeight: 400, color: theme.textMuted }}
        >
          /{target}
        </span>
      </div>
          ...
          {(!isRunning || !loaded) && (
            <div
              style={{
                ...
                zIndex: 2,
                width: '100%',
                aspectRatio: isRunning ? undefined : '16/9',
                display: 'flex',
                alignItems: 'center',
...
export default function GroundTruthRTSP({
  ...
  useEffect(() => {
    ...
    const run = async () => {
      try {
        ...
        console.log(
          '[RoomFetch] Harvesting allotments for dept=%s',
          roomDepartment,
        );
        ...
        const currentSess = (sessData?.uniqueSessions || []).find(
        ...
        const sems = (Array.isArray(semData) ? semData : [])
          .map((s) => s.sem)
          .filter(Boolean);
        if (sems.length === 0) {
          setTimetableRooms([]);
        ...
        const sessionStartYear =
          parseInt(currentSess.session.split('-')[0]) || 2025;

        const walk = (node, semDegree, calculatedBatchYear, semName) => {
          if (!node || typeof node !== 'object') return;
          if (Array.isArray(node)) {
            node.forEach((n) =>
              walk(n, semDegree, calculatedBatchYear, semName),
            );
            return;
          }
        ...
        await Promise.all(
          sems.map(async (sem) => {
            try {
              ...
              const calculatedBatchYear = String(
                sessionStartYear - (yearOfStudy - 1),
              );
              ...
              console.warn('[RoomFetch] Bypassed segment exception:', sem, e);
            }
          }),
        );
  ...
  const currentBatchRoomNames = useMemo(() => {
    return Array.from(
      new Set(
        timetableRooms
          .filter((r) => {
            // For department coordinator filters: Degree , Year
            if (fixedDepartment) {
              return (
                r.degree === degree && (year ? r.batchYear === year : true)
              );
            }
            // For admin filters: Degree only
            else {
  ...
  const allottedRooms = currentBatchRoomNames; // Rooms allocated to the Department( + Year if not admin)

  const otherRooms = useMemo(() => { // Other Rooms not allocated (Only for admin)
  return Array.from(
    new Set(
      timetableRooms
        .map((r) => r.roomName)
        .filter((roomName) => !allottedRooms.includes(roomName))
    )
  ).sort();
}, [timetableRooms, allottedRooms]);
  ...
  useEffect(() => {
    ...
    fetch(
      `${CAMERA_API}?roomId=${encodeURIComponent(selectedRoom.trim().toUpperCase())}`,
    )
      .then((r) => (r.ok ? r.json() : []))
      .then((data) => {
  ...
  const stopStream = useCallback(async () => {
    try {
      ...
      /* clean fallback trace exit */
    }
  }, [gtJobId]);

  const handleStart = useCallback(
    async (overrideCamera) => {
      if (!batchName) {
        showToast('Fill in Degree, Department and Year', 'error');
        return;
      }
      ...
      try {
        const previewRes = await Promise.race([
          ...
          new Promise((_, rej) =>
            setTimeout(() => rej(new Error('preview timeout')), 4000),
          ),
        ]);
        if (!previewRes.ok) {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          throw new Error(
            `Server error ${response.status}${text ? ': ' + text : ''}`,
          );
        }
            ...
            switch (ev.type) {
              ...
              case 'frame':
                if (ev.faces_this_frame > 0)
                  addLog(
                    `🎞 Frame ${ev.frame} — ${ev.faces_this_frame} face(s) detected`,
                    '#aaa',
                  );
                break;
              case 'person_update':
                ...
                if (ev.done)
                  addLog(
                    `✅ ${ev.person_id} reached ${ev.count} images`,
                    theme.success,
                  );
                break;
              case 'progress':
              ...
              case 'done':
                ...
                if (!combinedMode && !roomMode) {
                  showToast(
                    `${ev.people_detected} people — ${ev.images_saved} images saved`,
                  );
                }
                break;
              case 'job_id':
        ...
        if (err.name === 'AbortError') {
          ...
          addLog(
            `❌ ${err.message} — retrying in ${RETRY_DELAY}s (attempt ${attempt})…`,
            theme.danger,
          );
          setStatus('retrying');
  ...
  const handleStartRoomMode = useCallback(async () => {
    ...
    if (roomCameras.length === 0) {
      showToast('No cameras found for this room', 'error');
      return;
    }
  ...
  const totalImagesSession = Object.values(persons).reduce(
    (s, p) => s + (p.count || 0),
    0,
  );
    ...
    <div style={styles.page}>
      <div style={{ marginBottom: 28 }}>
        ...
        <div style={styles.subheading}>
          Acquire face images from a live RTSP camera stream → auto-stops when
          every detected person reaches the target image count
        </div>
      </div>
      ...
      >
        ...
        >
          ...
          {!fixedDepartment ? (
            ...
            // For Department Coordinator
            <div>
              <label style={styles.label}>Department</label>
              <div value={department} style={styles.select} disabled={true}>
                {deptLoading
                  ? 'Loading…'
                  : deptError
                    ? 'Error'
                    : fixedDepartment.replaceAll('_', ' ')}
              </div>
              {deptError && (
                <div
                  style={{
          ...
          {/* ONly show when department coordinator, not for admin */}
          {fixedDepartment && (
            <div>
        ...
        <div style={{ marginBottom: 20 }}>
          <label style={styles.label}>
            ...
            >
              — optional: automatically sourced from active session timetable
              allotments
            </span>
          </label>
              ...
              <>
                ...
                {fixedDepartment ? (
                  currentBatchRoomNames.length > 0 ? (
                    <optgroup
                      label={`── Allotted to ${degree} ${year ? 'Batch ' + year : ''}  ──`}
                    >
                    ...
                    <option disabled>
                      {' '}
                      No allotments registered for this configuration in active
                      session
                    </option>
                  )
                ) : (
                  <>
                    {allottedRooms.length > 0 ? (
                      <optgroup
                        label={`── Allotted to ${degree} ${department?.replaceAll('_', ' ') || ' '} (All Years) ──`}
                      >
                        {allottedRooms.map((room) => (
                          <option key={room} value={room}>
                            {room}
                          </option>
                        ))}
                      ...
                      <option disabled>
                        {' '}
                        No allotments registered for this configuration in
                        active session
                      </option>
                    )}
                    {otherRooms.length > 0 && (
            ...
            >
              {roomCamsLoad ? (
                ...
                <span style={{ color: '#0ea5e9' }}>
                  <strong>{roomCameras.length}</strong> camera
                  {roomCameras.length > 1 ? 's' : ''} routed — tap "Start"
                </span>
              ) : (
                <span style={{ color: theme.danger }}>
                  No cameras registered for room "{selectedRoom}"
                </span>
              )}
            </div>
          )}
              ...
              {CAMERAS.map((cam) => (
                <button
                  ...
                  style={{
                    ...
                    border: '1px solid',
                    borderColor:
                      cameraId === cam.id ? theme.accent : theme.border,
                    background:
                      cameraId === cam.id
                        ? theme.accentDim || theme.accent + '22'
                        : 'transparent',
        ...
        <div style={{ marginBottom: 20 }}>
          ...
          <div style={{ display: 'flex', gap: 8, marginTop: 6 }}>
            {TARGET_OPTIONS.map((opt) => (
              <button
                key={opt.value}
                onClick={() => setTargetImgs(opt.value)}
                title={opt.hint}
                style={{
                  padding: '7px 18px',
                  borderRadius: 6,
                  ...
                  border: '1px solid',
                  borderColor:
                    targetImgs === opt.value ? theme.accent : theme.border,
                  background:
                    targetImgs === opt.value
                      ? theme.accentDim || theme.accent + '22'
                      : 'transparent',
                  color:
                    targetImgs === opt.value ? theme.accent : theme.textMuted,
                  transition: 'all 0.15s',
                }}
              >
                {opt.value} imgs
          ...
          <div
            style={{ fontSize: '11px', color: theme.textMuted, marginTop: 5 }}
          >
            ...
            {DET_SIZE_OPTIONS.map((opt) => (
              <button
                ...
                style={{
                  ...
                  border: '1px solid',
                  borderColor:
                    detSize === opt.value ? theme.accent : theme.border,
                  background:
                    detSize === opt.value
                      ? theme.accentDim || theme.accent + '22'
                      : 'transparent',
            ...
            {FRAME_SKIP_OPTIONS.map((opt) => (
              <button
                ...
                style={{
                  ...
                  border: '1px solid',
                  borderColor:
                    frameSkip === opt.value ? theme.accent : theme.border,
                  background:
                    frameSkip === opt.value
                      ? theme.accentDim || theme.accent + '22'
                      : 'transparent',
                  color:
                    frameSkip === opt.value ? theme.accent : theme.textMuted,
                  transition: 'all 0.15s',
                }}
              >
                Every {opt.value}
        ...
        >
          ...
          <span
            style={{
              color: batchName ? theme.accent : theme.textMuted,
              fontWeight: 600,
            }}
          >
      ...
      >
        <button
          ...
          style={{
            ...styles.btnPrimary,
            opacity:
              isRunning || isStopping || isRetrying || !batchName || anyMode
                ? 0.5
                : 1,
            minWidth: 200,
          }}
          ...
          <button
            ...
            style={{
              ...
              background: combinedMode
                ? '#0284c7'
                : isRunning || isStopping || isRetrying || !batchName
                  ? '#94a3b8'
                  : '#0ea5e9',
              color: '#ffffff',
              boxShadow: combinedMode
        ...
        <button
          ...
          style={{
            ...
            border: 'none',
            background:
              isRunning || isRetrying || anyMode ? '#ef4444' : '#fca5a5',
            color: '#ffffff',
            boxShadow:
              isRunning || isRetrying || anyMode
      ...
      {combinedMode && isRunning && (
        ...
        >
          ...
          <div style={{ flex: 1 }}>
            ...
            <div
              style={{ fontSize: '11px', color: theme.textMuted, marginTop: 2 }}
            >
              Switching to{' '}
              {
                CAMERAS.find(
                  (c) =>
                    c.id ===
                    COMBINED_CAMERAS[
                      (combinedIdx + 1) % COMBINED_CAMERAS.length
                    ],
                )?.label
              }{' '}
              in_
      ...
      {isRetrying && (
        ...
        >
          ...
          <div style={{ flex: 1 }}>
            <div
              style={{ fontSize: '13px', fontWeight: 700, color: theme.danger }}
            >
      ...
      {(isRunning || isStopping || isRetrying || isDone || isError) && (
        ...
        >
          ...
          >
            <Dot
              color={
                isRunning ? theme.accent : isDone ? theme.success : theme.danger
              }
            ...
            >
              {isRunning &&
                `Acquiring from ${roomMode ? roomActiveCam?.label : selectedCamera?.label}…`}
          ...
          >
            ...
            {log.map((entry, i) => (
              <div
                key={i}
                style={{ display: 'flex', gap: 8, paddingBottom: 2 }}
              >
                <span style={{ color: '#555', flexShrink: 0 }}>
                  {entry.time}
                </span>
                <span style={{ color: entry.color || '#ccc' }}>
                  {entry.msg}
                </span>
              </div>
            ))}
      ...
      {isDone && summary && (
        ...
        >
          ...
          >
            Saved to:{' '}
            <span style={{ color: theme.accent }}>{summary.batchDir}</span>
          </div>
          <div
            style={{ marginTop: 12, fontSize: '13px', color: theme.textMuted }}
          >
            Folders named{' '}
            <strong style={{ color: theme.text }}>person_001</strong>,&nbsp;
            <strong style={{ color: theme.text }}>person_002</strong>, … — go
            to&nbsp;
      ...
      {isIdle && (
        ...
        >
          <div style={{ fontSize: '40px', marginBottom: 12, opacity: 0.4 }}>
            📡
          </div>
          ...
          <div style={{ fontSize: '13px', color: theme.textMuted }}>
            Select batch → optionally pick a room → set target images → click
            "Start Acquisition"
            <br />
            Stream stops automatically once every person reaches the target
            <br />
            <span style={{ color: '#0ea5e9' }}>🏫 Room mode</span> auto-switches
            between all cameras in the room every 5 min
            <br />
            <span style={{ color: '#f0c040' }}>🔄 Combined L ↔ R</span>{' '}
            alternates LT103L and LT103R every 5 min (manual mode)
          </div>
        </div>
      )}
    </div>
