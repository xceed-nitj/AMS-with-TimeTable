...
async function fetchUser(userid) {
    ...
    try {
        const response = await fetch(
            ...
            `${apiUrl}/api/v1/reviewmodule/user/getUser/${userid}`,
            // `${apiUrl}/api/v1/reviewmodule/user/getUsers`,
            {
                method:'GET',
                credentials: 'include'
            }
        ...
        if (response.status!=200) return false
        const data = await response.json();
        if(data) {console.log('data found');return data}
        else return false
...
const wrapper2 = async (userid) => console.log('fetching users',await fetchUser(userid))
const wrapper3 = async (setReviewerComments) => {console.log('attempting to fetch commments');setReviewerComments(await fetchReviewerComments())}
...
function PaperSummary() {
    ...
    // useEffect(()=>{if(paperData) wrapper2(paperData.authors[0])}, [paperData])
    useEffect(()=>{wrapper3(setReviewerComments)},[])
    ...
    return (
        ...
        <div style={{display:'flex', alignItems:'baseline', flexWrap:'wrap', maxWidth:"90%", margin: 'auto'}}>
            <div style={{width:(screenWidth>950)?'65%':'100%', display:'flex', height:'65vh', flexDirection:'column', margin:'10px', justifyContent:'space-between'}}>
                ...
                <Accordion width={'100%'} allowToggle>
                    ...
                    <AccordionItem>
                        ...
                        <AccordionPanel>
                            <Box>
                                <Textarea height={'25vh'} value={paperData && paperData.abstract} isDisabled style={{maxWidth:'800px', border: 'gray 1px solid'}}></Textarea>
                            </Box>
                        </AccordionPanel>
                    </AccordionItem>
                </Accordion>
                ...
                <div
                style={{backgroundColor:'#f8f8f8', border:'1px solid #f4f4f4', padding: '5px',margin: '10px', borderRadius:'10px', width:'100%'}}>
            ...
            <div style={{width:(screenWidth>950)?'30%':'100%', backgroundColor:'#f8f8f8', border:'1px solid #f4f4f4', padding: '5px',margin: 'auto', 
                borderRadius:'10px', display:'flex', flexDirection:'column', height:'65vh', alignItems:'center', justifyContent:'space-evenly'}}>
                <Heading textAlign={"center"}>STATUS</Heading>
                <Box bg={getStatusColor(paperData && paperData.status)} p={2} borderRadius="md" width={'40%'}>
