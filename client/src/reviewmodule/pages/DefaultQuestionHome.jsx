...
const ViewQuestions = () => {
  ...
  const fetchQuestions = async () => {
    try {
      const response = await axios.get(`${apiUrl}/reviewmodule/DefaultQuestion/all`);
  ...
  function DeleteQuestion(props) {
    ...
    const handleSubmit = async () =>{
      try {
        await axios.delete(`${apiUrl}/reviewmodule/DefaultQuestion/${props.id}`)
        toast({
      ...
      <Flex alignItems={'center'} width={'100%'}>
        ...
        <Flex style={{boxSizing:'border-box', width: '100%', justifyContent:'flex-end', flexGrow:'1', alignItems:'center'}}>
          <Button
            as={Link}
            style={{width: '80px'}}
            to={`/prm/DefaultQuestion?edit=${question._id}`} colorScheme='green'>
            Edit
          </Button>
 ...
 function sortQuestions(qArgs) {
  ...
  // Group questions by their order
  qArgs.forEach(question => {
    const orderValue = question.order[0];
    if (!orderMap.has(orderValue)) {
      orderMap.set(orderValue, []);
    }
    orderMap.get(orderValue).push(question);
  });

  // Flatten the grouped questions into a sorted array
  const sortedQuestions = [];
  Array.from(orderMap.entries())
    .sort((a, b) => a[0] - b[0])
    .forEach(([order, questions]) => {
      questions.forEach(question => sortedQuestions.push(question));
    });
