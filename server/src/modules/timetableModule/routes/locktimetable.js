...
LockTimeTableRouter.get("/viewsem/:code", async (req, res) => {
  try {
    ...
    const updatedTime = await locktimetableController.getLastUpdatedTimeByCode(
      code
    );
  ...
  async (req, res) => {
    try {
      ...
      res
        .status(200)
        .json({
          response: `Locked Time Table with code ${code} deleted successfully`,
        });
    } catch (error) {
