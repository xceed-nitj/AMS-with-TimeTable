...
subjectRouter.put("/:id", async (req, res) => {
  try {
    ...
    await subjectController.updateID(subjectId, updatedId);
...
subjectRouter.delete("/deletebycode/:code", async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ response: `Subjects with code ${code} deleted successfully` });
  } catch (error) {
    res.status(500).json({ error: "Internal Server Error" });
  }
});
...
subjectRouter.get("/firstyearsubject/:code/:dept", async (req, res) => {
  try {
    ...
    const subjects = await subjectController.getFirstYearDeptSubject(
      code,
      dept
    );
    res.status(200).json(subjects);
