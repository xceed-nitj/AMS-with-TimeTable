...
let tableControllerInstance;

function getTableControllerInstance() {
  if (!tableControllerInstance) {
...
class addFacultyController {
  async AddFaculty(req, res) {
    ...
    try {
      ...
      if (!existingFaculty) {
        const newFaculty = new addFaculty({
          code,
          sem,
          faculty: Array.isArray(faculty) ? faculty : [faculty],
        });
        ...
        res.json({
          message: "Faculty added to the existing semester successfully",
        });
      }
    } catch (error) {
      console.error(error);
  ...
  async getAddedFacultyByCode(code) {
    if (!code) {
  ...
  async deleteFirstYearDeptFaculty(code, sem, facultyname) {
    try {
      ...
      const tableController = getTableControllerInstance();
      const firstYear = await tableController.getCodeOfDept(
        "Basic Sciences",
        session
      );
      ...
      if (faculty) {
        ...
        faculty.faculty = faculty.faculty.filter(
    ...
    try {
      ...
      for (const item of faculty) {
        ...
        for (const facultyName of facultyValues) {
          const deptFaculty = await Faculty.findOne({
            name: facultyName,
            dept,
          });
